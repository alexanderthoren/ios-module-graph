...
| `quick_wins.py` | `compute_quick_wins` — per source folder: the **cut-set** blocking an extract-today (each edge classified `move_file`/`shared_primitive`/`invert`, generalizing `cycles.compute_extraction_targets` beyond cycles), the auto-picked **absorb-into-existing** SPM destination (default outcome; rejected on module-cycle risk or the `ABSORB_MAX_*` complexity bounds in `config.py`), ranked by `scoring` ROI; ships as `payload["quick_wins"]` |
| `file_affinity.py` | `compute_file_moves` — misplaced-file detector over `file_edges`: suggest moving a file when one foreign folder holds ≥`FILE_AFFINITY_MIN_REFS` of its reference mass and dominates every alternative (own folder included) ≥`FILE_AFFINITY_DOMINANCE`×; advisory, smallest possible PRs, dissolves fake coupling pre-extraction; ships as `payload["file_moves"]` |
| `isolate.py` | `compute_isolation_targets`/`compute_isolations` — within ONE module, rank its declared types by reference mass (external + internal) for pull-into-own-module: each seed (≥`ISOLATE_MIN_EXT_REFS` external refs) carries the **drag closure** (in-module types it transitively uses → must move with it; computed over `type_edges`' `"name\tfolder"` tokens), the distinct build modules it would **free** (via `module_graph.module_of`), the closure's public surface, and an ROI (external mass freed / module size). `isolatable_modules` gates the prefixes to real compile units (SPM target roots + dividable app subtrees), excluding aggregating ancestors. Index path only (no `type_edges` on the scan path); ships as `payload["isolations"]`, powers the UI's "Isolate" action (companion to `divide`) |
| `objective.py` | `compute_cost_model` (pure, node/edge-agnostic) + `compute_objective` — the **expected-build-cost** pricer: `warm_cost` = Σ churn(m)×rebuild_work(m) (the churn window's recompile bill; churn-less fallback weighs every module 1), `cold_cost` = work-weighted longest depends-on chain, `efficiency` = resource floor ÷ wall floor (dependency-bound vs resource-bound). Work = measured seconds when available else type counts (`unit` says which). Prices simulated graphs identically — the comparator every other score reduces to |
| `partition.py` | `compute_partition`/`compute_partitions` — **usage-cohort module splitting** (the successor to folder-based `divide` for real modules): SCC-condense the intra-module type graph (mutual refs are welded — escape hatch is an inversion, reported not assumed), one drag closure per external consumer boundary, types reachable from ≥2 cohorts → shared **core**, exclusive remainders are disjoint and can only reference their own cohort or the core ⇒ quotient acyclic **by construction**. Identical exclusives merge; slices <`PARTITION_MIN_PART_TYPES` fold into the rump; core >`PARTITION_CORE_MAX_PCT`% ⇒ verdict `no_seam` **with the blocking hub types** (the actionable output — isolate/invert them to create the seam). `ok` partitions are priced via `objective.compute_cost_model` against the module graph (warm/cold deltas in `sim`). Index path only; ships as `payload["partitions"]` |
| `advisor.py` | `compute_advice` — the **arbiter**: merges every advice stream (`file_moves`, `quick_wins`, `isolations`, `partitions`, `module_splits`, `recommendations`) into ONE wave-ordered action feed with an explicit stop line. Waves: 0 file moves → 1 ready extractions → 2 extractions unblocked by those very moves (cross-linked `after` the move ids) + small manual cuts (≤`ADVISOR_CUT_MAX` refs) → 3 module surgery (ONE decision per module: split with releasable consumers > dominant isolation ≥`ADVISOR_ISOLATE_SHARE`% > **usage-cohort partition** (verdict `ok`) > folder division; runner-up kept as `alternative`) **+ `api_retrofit`** (existing SPM module with ≥`API_MIN_CONSUMERS` direct consumers and no `<Name>API` — the action the api_coverage criterion was missing) → 4 joins (tiny ≤`ADVISOR_JOIN_MAX_TYPES`-type SPM module with exactly one SPM consumer → fold in; `*API`-labelled modules are **exempt** — a fresh API package with one consumer is the convention working). Anything under `ADVISOR_TAIL_PCT`% of its kind's best payoff lands in `deferred[]` with a machine `reason` (`tail`/`blocked_cut`/`no_seam`/`stabilize_api`/`unused`) + human `why`. Pure interpretation, deterministic. Wrapped by `master_plan.py` into `payload["master_plan"]` |
| `master_plan.py` | `compute_master_plan` — Migration mode's whole data source (wraps `advisor.compute_advice`): every action becomes a full **step** with a **shape decision** + the rule that fired (`api_impl` pair `<Name>API`+`<Name>` when ≥`API_MIN_CONSUMERS` current-or-future consumer modules OR churn-hot×warm-wide OR on the cold critical path; `single_module`; quick-win `absorb`; `api_retrofit`; `partition`), **cumulative simulated deltas** (`_PlanSim` replays the whole plan move by move — extractions, API rewires, retrofits, joins — over the folder edges; step N is priced against the world steps 1…N−1 produced, costs via `objective.compute_cost_model` in type-units), a per-step **verify block**, a **trajectory** (per-step rows + projected end state + `equilibrium["projected"]`), a detection-driven **setup checklist**, and **equilibrium criteria** (`EQ_APP_SHARE_PCT` app share; warm bound **churn-aware** when churn data exists — only churn-hot modules count against `EQ_WARM_MAX_PCT`; zero module cycles; API coverage; `EQ_PAR_EFF_PCT` cold parallelism efficiency when measured; empty feed). Scan path degrades and says so. Setup items ship as ordinary phase −1 steps inside `steps` (one feed, no separate surface; the raw `setup` list is also kept). Ships as `payload["master_plan"]`; renders as Migration mode's single Plan surface and `migration_plan.md` |
| `evaluate.py` | `compute_evaluation` + `python3 -m modgraph.evaluate <root> <index_graph.json> [out.md]` — the **plan-policy comparator**: replays `master` vs `no_api` (same actions, API decisions stripped) vs `topo` (pre-advisor bottom-up everything) through the same `_PlanSim` + cost model; reports end state, value after `EVAL_EARLY_STEPS`, and a uniform effort proxy. Real-data result (Fever + IceCubes): master lowers warm cost; **no_api makes the same extractions warm-WORSE than baseline** (+15%/+9%) and topo explodes it (+78%) — the API-firewall decision is what makes modularization pay |
...
| `tasks.py` | `master_plan_markdown`, `write_master_plan_markdown`, `write_master_plan_json` — `migration_plan.md` is the file-shaped twin of the Plan tab (same `master_plan` input, so CLI and UI can never disagree): Setup checklist, phase-grouped steps with shape/why/verify, stop line + deferred-with-reasons, equilibrium checklist, blocked section |
