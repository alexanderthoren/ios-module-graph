...
| `advisor.py` | `compute_advice` — the **arbiter**: merges every advice stream (`file_moves`, `quick_wins`, `isolations`, `module_splits`, `recommendations`) into ONE wave-ordered action feed with an explicit stop line. Waves: 0 file moves → 1 ready extractions → 2 extractions unblocked by those very moves (cross-linked `after` the move ids) + small manual cuts (≤`ADVISOR_CUT_MAX` refs) → 3 module surgery (ONE decision per module: split with releasable consumers > dominant isolation ≥`ADVISOR_ISOLATE_SHARE`% > division plan; runner-up kept as `alternative`) → 4 joins (tiny ≤`ADVISOR_JOIN_MAX_TYPES`-type SPM module with exactly one SPM consumer → fold in). Anything under `ADVISOR_TAIL_PCT`% of its kind's best payoff (isolations: roi) lands in `deferred[]` with a machine `reason` + human `why` — the stop line is the actions/deferred boundary. Pure interpretation over already-computed dicts, no recompute; deterministic. No longer shipped directly — `master_plan.py` wraps it (wrap, don't fork) into `payload["master_plan"]` |
| `master_plan.py` | `compute_master_plan` — Migration mode's whole data source (wraps `advisor.compute_advice`): every action becomes a full **step** with a **shape decision** + the rule that fired (`api_impl` pair `<Name>API`+`<Name>` when ≥`API_MIN_CONSUMERS` current-or-future consumer modules OR churn-hot×warm-wide OR on the cold critical path; `single_module` when an API split is a boundary wave 4 would later fold; quick-win `absorb` otherwise; API surface/`protocols_for` from `pair_types`/`type_kinds` — classes/actors become protocols, value types move whole), **simulated post-step deltas** (`_sim_extract` replays the move over module edges and re-runs `compute_build_impact`: module-count + crit-len into `verify.expect`, estimated work leaving the app target when measured), a per-step **verify block**, a detection-driven **setup checklist** (packages layout / API convention / composition root / CI ratchet / measured baseline / exclusions review — emitted only when the data says they're missing), and **equilibrium criteria** (`EQ_APP_SHARE_PCT` app share, `EQ_WARM_MAX_PCT` warm bound, zero module cycles, API coverage for multi-consumer modules, empty feed) — the measurable definition of done. Scan path degrades to `single_module`/`absorb` with empty surface and says so. Ships as `payload["master_plan"]` (replaced `advice`); renders as Migration mode's two tabs (Setup + Plan) and `migration_plan.md` |
