...
def _cost(node: dict) -> float:
    """Compile-cost proxy: measured build time (ms) when available, else types."""
    if node.get("measured") and node.get("build_ms"):
        return float(node["build_ms"])
    return float(node.get("types") or node.get("folders") or 1)


def _fmt(cost: float, measured: bool) -> str:
    """Human-readable cost: seconds when measured (cost is ms), else type count."""
    if measured:
        return f"{cost / 1000:.1f}s"
    return f"{int(cost)} types"
...
def compute_split_recommendations(module_graph: dict, divisions: dict | None = None) -> dict:
    ...
    measured = any(n.get("measured") for n in nodes)
    ...
    for n in nodes:
        ...
        raw.append({
            ...
            "downstream_human": _fmt(downstream_cost, measured),
            "crit": crit,
...
def _advise(r: dict) -> tuple[str, str]:
    ...
    if r["dependents"]:
        parts.append(f"editing it recompiles {r['dependents']} module(s)"
                     f" (~{r['downstream_human']} of downstream build)")
    if r["crit"]:
        parts.append(f"on the cold critical path (+{r['cost_human']} to the floor;"
